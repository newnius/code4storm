 ...
 *
 */
public class MysqlCategoryReader implements IRichSpout{
    ...
    public void nextTuple() {
        ...
        try {
            String sql = "select category_id, category_name  from `used_category` limit " +offset +", " + batchSize;
            ...
            while(rs.next()){
                ...
                cnt++;
                collector.emit(new Values(category), category);
            }
            statement.close();
            if(cnt == batchSize) {
                offset += batchSize;
    ...
    public void ack(Object msgId) {
        logger.info("ack "+ ((Category) msgId).getName());
    }

    @Override
    public void fail(Object msgId) {
        collector.emit(new Values((Category)msgId), msgId);
        logger.info("fail "+ ((Category) msgId).getName());
    }
